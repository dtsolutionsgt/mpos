...
public class clsP_res_sesionObj {
    ...
    private String sel="SELECT * FROM P_res_sesion";
    private String sql;
    public ArrayList<clsClasses.clsP_res_sesion> items= new ArrayList<clsClasses.clsP_res_sesion>();

    public clsP_res_sesionObj(Context context, BaseDatos dbconnection, SQLiteDatabase dbase) {
        cont=context;
        Con=dbconnection;
    ...
    public void add(clsClasses.clsP_res_sesion item) {
        addItem(item);
    }

    public void update(clsClasses.clsP_res_sesion item) {
        updateItem(item);
    }

    public void delete(clsClasses.clsP_res_sesion item) {
        deleteItem(item);
    }
    ...
    public clsClasses.clsP_res_sesion first() {
        return items.get(0);
    }
    ...
    private void addItem(clsClasses.clsP_res_sesion item) {
    ...
    private void updateItem(clsClasses.clsP_res_sesion item) {

        upd.init("P_res_sesion");
    ...
    private void deleteItem(clsClasses.clsP_res_sesion item) {
        sql="DELETE FROM P_res_sesion WHERE (ID='"+item.id+"')";
        db.execSQL(sql);
    }

    private void deleteItem(String id) {
        sql="DELETE FROM P_res_sesion WHERE id='" + id+"'";
        db.execSQL(sql);
    }

    private void fillItems(String sq) {
        Cursor dt;
        clsClasses.clsP_res_sesion item;
    ...
    public String updateItemSql(clsClasses.clsP_res_sesion item) {

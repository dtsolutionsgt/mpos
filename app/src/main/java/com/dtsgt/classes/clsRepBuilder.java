...
//	}
    ...
    public void add3Tot(int s1,double v2,double v3,double v4) {
        ...
        ts=ltrim(Integer.toString(s1),prwt-6)+ltrim(sval,prwt+1)+ltrim(sval2,prwt-val)+ltrim(sval3,prwt);
        items.add(ts);
    }
	...
	public void add4lrrTotPorc(String s1,String s2,double s3,double v3) {
		...
		tot=sval.length();
		ts=ltrim(s1,prwt)+ltrim(s2,prwt-5)+rtrim(stot,prwt+1)+rtrim(sval,prwt-5);
		items.add(ts);
	}
	...
	public void add4(double s1,double s2,double s3,double v3) {
        ...
        tot=sval.length()-2;
		ts=ltrim(s1tot,prwt)+rtrim(s2tot,prwt-4)+rtrim(s3tot,prwt)+rtrim(sval,prwt-4);
        items.add(ts);
    }

	public void add4rrrr(double v1,double v2,double v3,double v4) {
		String s1,s2,s3,s4;
		String stot;
		...
		s4=cursym+decfrm.format(v4);

		stot=rtrim(s1,prwq-1)+" "+rtrim(s2,prwq-1)+" "+rtrim(s3,prwq-1)+" "+rtrim(s4,prwq-1);
		items.add(stot);
	}

	public void add4rrrr(String s1,String s2,String s3,String s4) {
		String stot;
		stot=rtrim(s1,prwq-1)+" "+rtrim(s2,prwq-1)+" "+rtrim(s3,prwq-1)+" "+rtrim(s4,prwq-1);
		items.add(stot);
	}
	...
	public void add3lrres(String s1, String s2, String s3) {
		ts=ltrim(s1,prwt-4)+rtrim(s2,prwt+2)+rtrim(s3,prwt+4);
		items.add(ts);
	}
	...
	public void addtotsph(String s1,double val) {
		String sval;
		int size = 12;
	...
	public void addtotsppr(String s1,double val) {
		String sval;
		int size = 12;
	...
	public void addtotsptic(String s1,double val) {
		...
		sval=cursym+decfrm.format(val);
		ts=ltrim(s1,prw-14)+""+rtrim(sval,13);
		items.add(ts);
	}

	public void addtotsptic(String s1,int val) {
		String sval;
		int size = 12;
		...
		ts=ltrim(s1,prw-14)+""+rtrim(""+val,13);
		items.add(ts);
	}

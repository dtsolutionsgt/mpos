...
public class fbPStockTransExec extends fbBase {
    ...
    private String idtrans,transerr;
    private int idsuc;

    public fbPStockTransExec(String troot, int idsucursal) {
        super(troot);
        idsuc=idsucursal;
    }
    ...
    public void processTrans(String idtransaction,Runnable rnCallback) {
        idtrans=idtransaction;
        callBack=rnCallback;
        getTransItem("/"+idsuc+"/"+idtrans,rnCallback);
    }

    public void getTransItem(String idtrans,Runnable rnCallback) {

        fdb.getReference(root+"/"+idsuc+"/"+idtrans).get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            @Override
            public void onComplete(@NonNull Task<DataSnapshot> task) {
                if (task.isSuccessful()) {
                    DataSnapshot res=task.getResult();

                    clsClasses.clsFbPStock sitem=clsCls.new clsFbPStock();

                    sitem.id=res.child("id").getValue(Integer.class);
                    sitem.idalm=res.child("idalm").getValue(Integer.class);
                    sitem.cant=res.child("cant").getValue(Double.class);
                    sitem.nombre =res.child("nombre").getValue(String.class);
                    sitem.um =res.child("um").getValue(String.class);

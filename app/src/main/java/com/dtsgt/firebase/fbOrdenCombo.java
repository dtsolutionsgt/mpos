...
public class fbOrdenCombo extends fbBase {
    ...
    public void listItems(String orden,int combo,Runnable rnCallback) {
        try {
            idorden=orden;idcombo=combo;
    ...
    public void listItemsActive(String orden,Runnable rnCallback) {
        try {
            idorden=orden;
            items.clear();errflag=false;error="";
                    ...
                    get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
                        ...
                        public void onComplete(@NonNull Task<DataSnapshot> task) {
                            ...
                            if (task.isSuccessful()) {
                                ...
                                if (res.exists()) {
                                    for (DataSnapshot rsnap : res.getChildren()) {
                                        for (DataSnapshot snap : rsnap.getChildren()) {

                                            try {

                                                idsel = snap.child("idseleccion").getValue(Integer.class);

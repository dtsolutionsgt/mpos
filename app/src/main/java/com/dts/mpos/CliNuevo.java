...
public class CliNuevo extends PBase {
	...
	public void doSave(View view) {
		...
		try {
			...
			msgAskSave("Crear cliente nuevo y vender");
		} catch (Exception e) {
			addlog(new Object() {
	...
	private void saveData(int vender) {
		...
		*/
			...
			if (gl.peModal.equalsIgnoreCase("APR")) {
				...
				if (vender==1) {
					gl.credito = 0;
					gl.cliente = corel;
					startActivity(new Intent(this, CliDet.class));
				} else {
					toast("Cliente nuevo creado");
				}
				super.finish();
			}
	...
	private void msgAskSave(String msg) {
		try{
			...
			dialog.setPositiveButton("Vender", new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int which) {
					saveData(1);
				}
			});

			dialog.setNeutralButton("Solo crear", new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int which) {
					saveData(0);
				}
			});
